...
const ProjectPhases = ({projectId, projectMaxGrade, phases: initialPhases, onRefresh}) => {
    ...
    const [newPhase, setNewPhase] = useState({
        phase_num: '', phase_name: '', description: '', phase_load: '', deadline: '',
    });
    ...
    const handlePhaseSubmit = async (e) => {
        e.preventDefault();
        try {
            const res = await addPhase(newPhase.phase_num, newPhase.phase_name, newPhase.description, newPhase.phase_load, newPhase.deadline, projectId);

            if (res.status === 200) {
                showAlert({
                    message: res.message, severity: "success"
                });
                onRefresh();
                setIsOpen(false);
                setNewPhase({
    ...
    const confirmDelete = async () => {
        try {
            ...
            if (res.status === 200) {
                showAlert({
                    message: "Phase deleted successfully", severity: "success"
                });
                onRefresh();
            } else {
                throw new Error(res.message);
    ...
    const handleEditSubmit = async (e) => {
        e.preventDefault();
        try {
            const res = await editPhase(editPhaseData.phase_num, editPhaseData.phase_name, editPhaseData.description, editPhaseData.phase_load, editPhaseData.deadline, projectId);
    ...
    return (<div className="mt-6">
        <Card className="shadow-lg">
            ...
            <CardContent>
                <div className="space-y-4">
                    {phases.map((phase) => (<div key={phase.phase_num} className="border p-4 rounded-lg">
                        <div className="flex justify-between items-start">
                            <div>
                                <h3 className="font-semibold">Phase {phase.phase_num}: {phase.phase_name}</h3>
                                <p className="text-sm text-gray-600">{phase.description}</p>
                                <div className="mt-2 text-sm">
                                    <p>Grade: {projectMaxGrade * Number(phase.phase_load) / 100}</p>
        ...
        <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
            <DialogContent>
                ...
                <p>Are you sure you want to delete phase {phaseToDelete?.phase_name}? This action cannot be
                   undone.</p>
                <DialogFooter>
