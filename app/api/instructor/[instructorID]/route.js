...
// Get course details including instructor, projects with phases and submissions, and assignments with submissions
export async function GET(request, { params }) {
  try {
    ...
    // Fetch projects for the course
    const projectsResult = await pool.query(
      `
      SELECT p.project_id, p.project_name, COALESCE(p.start_date::TEXT, '') AS start_date, 
      COALESCE(p.end_date::TEXT, '') AS end_date, p.description, COALESCE(p.max_team_size, 0) AS max_team_size, COALESCE(p.max_grade, 0) AS max_grade
      FROM Project p
      ...
      projectsResult.rows.map(async (project) => {
        ...
        const phasesResult = await pool.query(
          ...
          SELECT ph.phase_num, ph.phase_name, COALESCE(ph.deadline::TEXT, '') AS deadline, 
                 COALESCE(ph.phase_load, 0) AS phase_load
          FROM Phase ph
    ...
    const assignmentsResult = await pool.query(
      `
      SELECT a.assignment_id, a.title, COALESCE(a.due_date::TEXT, '') AS due_date, a.description, COALESCE(a.max_grade, 0) AS max_grade
      FROM Assignment a
