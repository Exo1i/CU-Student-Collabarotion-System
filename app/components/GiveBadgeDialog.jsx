    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (result.status === 201) {
                ...
                onBadgeAdded(badges?.filter(badge => badge.badge_id === badgeId)[0])
            } else {
                setNotification({
                    type: "error",
                                ...
                                {badges.length > 0 ? (
                                    <div className="grid gap-4">
                                        {badges.map((badge) => (
                                            ...
                                            >
                                                <img src={badge.picture} alt={badge.title} className="w-10 h-10" />
                                                <div>
                                    ...
                                    <p className="text-center text-sm text-gray-500">No badges available</p>
                                )}
                            </div>
                        </div>
                        <DialogFooter>
                            <Button type="submit" disabled={!badgeId}>Assign Badge</Button>
                        </DialogFooter>
                    </form>
                </DialogContent>
            </Dialog>
