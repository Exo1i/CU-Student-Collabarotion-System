...
export async function addProject(name, coursecode, endDate, description, maxSize, maxgrade) {
    console.log('Received parameters:', { name, coursecode, endDate, description, maxSize, maxgrade });

    // Validate input more rigorously
    if (!name) {
        console.error('Missing required parameters');
        return {
            status: 422, message: 'Invalid project data'
        };
    }
    ...
    try {
        ...
        return {
            status: 200, message: "Project added successfully",
        };
    } catch (err) {
        ...
        if (err.code === '23000') {  // Refrential integrity violation
            return {
