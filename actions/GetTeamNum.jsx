...
export async function getUserTeamNum(student_ID, Project_ID) {
    ...
    try {
        const result = await pool.query(`SELECT team_num FROM PARTICIPATION WHERE student_id = $1 AND project_id = $2`, [student_ID, Project_ID]);

        // Check if no rows found
        if (result.rows.length === 0) {
            return {
                status: 404, message: "No team found for this student in this project",
            };
        }

        return {
            status: 200, message: result.rows[0].team_num,
        };
    } catch (err) {
